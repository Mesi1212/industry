...
from start.models import Courses, SessionYearModel, Subject, Student
...
class AddStudentForm(forms.Form):
    email = forms.EmailField(label="Email", max_length=50, widget=forms.EmailInput(attrs={"class": "form-control", "autocomplete": "off"}), required=True)
    ...
    username = forms.CharField(label="Username", max_length=50, widget=forms.TextInput(attrs={"class": "form-control", "autocomplete": "off"}), required=True)
    address = forms.CharField(label="Address", max_length=50, widget=forms.TextInput(attrs={"class": "form-control"}), required=True)
    profile_pic = forms.FileField(label="Profile Pic", max_length=50, widget=forms.FileInput(attrs={"class": "form-control"}), required=True)

    course = forms.ChoiceField(label="Course", choices=[], widget=forms.Select(attrs={"class": "form-control"}), required=True)

    gender_choice = (("Male", "Male"), ("Female", "Female"))
    sex = forms.ChoiceField(label="Sex", choices=gender_choice, widget=forms.Select(attrs={"class": "form-control"}), required=True)

    session_start_year = forms.ChoiceField(label="Session Start Year", choices=[], widget=forms.Select(attrs={"class": "form-control"}), required=True)
    session_end_year = forms.ChoiceField(label="Session End Year", choices=[], widget=forms.Select(attrs={"class": "form-control"}), required=True)

    def __init__(self, *args, **kwargs):
        super(AddStudentForm, self).__init__(*args, **kwargs)
        self.fields['course'].choices = [(course.id, course.course_name) for course in Courses.objects.all()]
        session_years = SessionYearModel.objects.all()
        self.fields['session_start_year'].choices = [(ses.session_start_year, ses.session_start_year) for ses in session_years]
        self.fields['session_end_year'].choices = [(ses.session_end_year, ses.session_end_year) for ses in session_years]
